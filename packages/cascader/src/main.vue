<template>
    <div
        ref="reference"
        :class="['aex-cascader', realSize && `aex-cascader--${realSize}`, { 'is-disabled': isDisabled }]"
    ...
    >
        ...
        >
            <template slot="suffix">
                <i
                    ...
                    key="clear"
                    class="aex-input__icon aex-icon-error"
                    @click.stop="handleClear"
                ></i>
                <i
                    v-else
                    key="arrow-down"
                    :class="['aex-input__icon', 'aex-icon-down', dropDownVisible && 'is-reverse']"
                    @click.stop="toggleDropDownVisible()"
                ></i>
            </template>
        </aex-input>

        <div v-if="multiple" class="aex-cascader__tags">
            <aex-tag
            ...
            <input
                ...
                type="text"
                class="aex-cascader__search-input"
        ...
        <transition name="aex-zoom-in-top" @after-leave="handleDropdownLeave">
            <div v-show="dropDownVisible" ref="popper" :class="['aex-popper', 'aex-cascader__dropdown', popperClass]">
                <aex-cascader-panel
                    ref="panel"
                    v-show="!filtering"
                ...
                <aex-scrollbar
                    ...
                    v-show="filtering"
                    tag="ul"
                    class="aex-cascader__suggestion-panel"
                ...
                >
                    <template v-if="suggestions.length">
                        <li
                            ...
                            :key="item.uid"
                            :class="['aex-cascader__suggestion-item', item.checked && 'is-checked']"
                            :tabindex="-1"
                            @click="handleSuggestionClick(index)"
                        >
                            <span>{{ item.text }}</span>
                            <i v-if="item.checked" class="aex-icon-check"></i>
                        </li>
                    </template>
                    <slot v-else name="empty">
                        <li class="aex-cascader__empty-text">{{ t('aex.cascader.noMatch') }}</li>
                    </slot>
                </aex-scrollbar>
            </div>
        </transition>
...
import AexTag from 'aex-ui/packages/tag';
import AexScrollbar from 'aex-ui/packages/scrollbar';
import AexCascaderPanel from 'aex-ui/packages/cascader-panel';
import AriaUtils from 'aex-ui/src/utils/aria-utils';
import { t } from 'aex-ui/src/locale';
import { isEqual, isEmpty, kebabCase } from 'aex-ui/src/utils/util';
import { isUndefined, isFunction } from 'aex-ui/src/utils/types';
import { isDef } from 'aex-ui/src/utils/shared';
import { addResizeListener, removeResizeListener } from 'aex-ui/src/utils/resize-event';
...
export default {
    ...
    props: {
        ...
        placeholder: {
            type: String,
            default: () => t('aex.cascader.placeholder'),
        },
        disabled: Boolean,
        clearable: Boolean,
        filterable: Boolean,
    ...
    computed: {
        realSize() {
            const _aexFormItemSize = (this.aexFormItem || {}).aexFormItemSize;
    ...
    methods: {
        ...
        focusFirstNode() {
            this.$nextTick(() => {
                ...
                if (filtering && suggestionPanel) {
                    ...
                    const firstMenu = popper.querySelector('.aex-cascader-menu');
        ...
        updateStyle() {
            ...
            const inputInner = $el.querySelector('.aex-input__inner');

            if (!inputInner) return;

            const tags = $el.querySelector('.aex-cascader__tags');
            let suggestionPanelEl = null;

            if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
                const suggestionList = suggestionPanelEl.querySelector('.aex-cascader__suggestion-list');

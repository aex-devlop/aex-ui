<template>
	<div
		...
		:class="[
			{
				'aex-table--fit': fit,
				'aex-table--striped': stripe,
				'aex-table--border': border || isGroup,
				'aex-table--hidden': isHidden,
				'aex-table--group': isGroup,
				'aex-table--fluid-height': maxHeight,
				'aex-table--scrollable-x': layout.scrollX,
				'aex-table--scrollable-y': layout.scrollY,
				'aex-table--enable-row-hover': !store.states.isComplex,
				'aex-table--enable-row-transition':
					(store.states.data || []).length !== 0 &&
					(store.states.data || []).length < 100
			},
			tableSize ? `aex-table--${tableSize}` : ''
		]"
	...
	>
		...
		<div
			...
			class="aex-table__header-wrapper"
			ref="headerWrapper"
		>
			<table-header
				ref="tableHeader"
		...
		>
			...
			<div
				v-if="$slots.append"
				class="aex-table__append-wrapper"
				ref="appendWrapper"
			>
		...
		<div
			...
			v-mousewheel="handleHeaderFooterMousewheel"
			class="aex-table__footer-wrapper"
			ref="footerWrapper"
		>
			<table-footer
				:store="store"
				:border="border"
				:sum-text="sumText || t('aex.table.sumText')"
		...
		>
			<div
				v-if="showHeader"
				class="aex-table__fixed-header-wrapper"
			...
			<div
				class="aex-table__fixed-body-wrapper"
			...
			>
				...
				<div
					v-if="$slots.append"
					class="aex-table__append-gutter"
			...
			<div
				v-if="showSummary"
				v-show="data && data.length > 0"
				class="aex-table__fixed-footer-wrapper"
		...
		<div
			v-if="rightFixedColumns.length > 0"
			class="aex-table__fixed-right-patch"
			ref="rightFixedPatch"
			:style="{
		...
		></div>
		<div class="aex-table__loading-mask" v-show="loading">
			<i class="aex-icon-loading" />
		</div>
		<div
			class="aex-table__column-resize-proxy"
			ref="resizeProxy"
...
import Mousewheel from 'aex-ui/src/directives/mousewheel';

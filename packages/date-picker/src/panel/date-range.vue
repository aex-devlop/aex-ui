<template>
	<transition name="aex-zoom-in-top" @after-leave="$emit('dodestroy')">
		<div
			v-show="visible"
			class="aex-picker-panel aex-date-range-picker aex-popper"
			:class="[
				{
		...
		>
			<div class="aex-picker-panel__body-wrapper">
				<slot name="sidebar" class="aex-picker-panel__sidebar"></slot>
				<div class="aex-picker-panel__sidebar" v-if="shortcuts">
					<button
						type="button"
						class="aex-picker-panel__shortcut"
				...
				<div class="aex-picker-panel__body">
					<div class="aex-date-range-picker__time-header" v-if="showTime">
						<span class="aex-date-range-picker__editors-wrap">
							<span class="aex-date-range-picker__time-picker-wrap">
								<aex-input
									size="small"
									:disabled="rangeState.selecting"
									ref="minInput"
									:placeholder="t('aex.datepicker.startDate')"
									class="aex-date-range-picker__editor"
									:value="minVisibleDate"
									@input="(val) => handleDateInput(val, 'min')"
									@change="(val) => handleDateChange(val, 'min')"
								/>
							</span>
							<span class="aex-date-range-picker__time-picker-wrap" v-clickoutside="handleMinTimeClose">
								<aex-input
									size="small"
									class="aex-date-range-picker__editor"
									:disabled="rangeState.selecting"
									:placeholder="t('aex.datepicker.startTime')"
									:value="minVisibleTime"
									:suffix-icon="!!minTimePickerVisible ? 'aex-icon-up-caret' : 'aex-icon-down-caret'"
						...
						<span class="aex-icon-right"></span>
						<span class="aex-date-range-picker__editors-wrap is-right">
							<span class="aex-date-range-picker__time-picker-wrap">
								<aex-input
									size="small"
									class="aex-date-range-picker__editor"
									:disabled="rangeState.selecting"
									:placeholder="t('aex.datepicker.endDate')"
							...
							<span class="aex-date-range-picker__time-picker-wrap" v-clickoutside="handleMaxTimeClose">
								<aex-input
									size="small"
									class="aex-date-range-picker__editor"
									:disabled="rangeState.selecting"
									:placeholder="t('aex.datepicker.endTime')"
									:value="maxVisibleTime"
									:suffix-icon="!!maxTimePickerVisible ? 'aex-icon-up-caret' : 'aex-icon-down-caret'"
									:readonly="!minDate"
					...
					<div class="aex-picker-panel__content aex-date-range-picker__content is-left">
						<div class="aex-date-range-picker__header">
							<button type="button" @click="leftPrevYear" class="aex-picker-panel__icon-btn aex-icon-left-double"></button>
							<button type="button" @click="leftPrevMonth" class="aex-picker-panel__icon-btn aex-icon-left"></button>
							<button
								type="button"
								@click="leftNextYear"
								v-if="unlinkPanels"
								...
								class="aex-picker-panel__icon-btn aex-icon-right-double"
							></button>
							<button
								type="button"
								@click="leftNextMonth"
								...
								class="aex-picker-panel__icon-btn aex-icon-right"
							></button>
							<div>{{ leftLabel }}</div>
						</div>
						<date-table
					...
					<div class="aex-picker-panel__content aex-date-range-picker__content is-right">
						<div class="aex-date-range-picker__header">
							<button
								type="button"
								@click="rightPrevYear"
								...
								:disabled="!enableYearArrow"
								:class="{ 'is-disabled': !enableYearArrow }"
								class="aex-picker-panel__icon-btn aex-icon-left-double"
							></button>
							<button
								type="button"
								...
								:disabled="!enableMonthArrow"
								:class="{ 'is-disabled': !enableMonthArrow }"
								class="aex-picker-panel__icon-btn aex-icon-left"
							></button>
							<button type="button" @click="rightNextYear" class="aex-picker-panel__icon-btn aex-icon-right-double"></button>
							<button type="button" @click="rightNextMonth" class="aex-picker-panel__icon-btn aex-icon-right"></button>
							<div>{{ rightLabel }}</div>
						</div>
						<date-table
							selection-mode="range"
			...
			<div class="aex-picker-panel__footer" v-if="showTime">
				<aex-button size="mini" type="text" class="aex-picker-panel__btn" @click="handleClear">
					{{ t('aex.datepicker.clear') }}
				</aex-button>
				<aex-button plain size="mini" class="aex-picker-panel__btn" :disabled="btnDisabled" @click="handleConfirm(false)">
...
export default {
	...
	computed: {
		...
		leftLabel() {
			return (
				...
				' ' +
				this.t('aex.datepicker.year') +
				' ' +
				this.t(`aex.datepicker.month${this.leftDate.getMonth() + 1}`)
			);
		},
		...
		rightLabel() {
			return (
				...
				this.t('aex.datepicker.year') +
				' ' +
				this.t(`aex.datepicker.month${this.rightDate.getMonth() + 1}`)
			);
		},
	...
	components: { TimePicker, DateTable, AexInput, AexButton }
};
</script>

<template>
    <div :class="['aex-cascader-panel', border && 'is-bordered']" @keydown="handleKeyDown">
        <cascader-menu
            ref="menu"
...
import { noop, coerceTruthyValueToArray, isEqual, isEmpty, valueEquals } from 'aex-ui/src/utils/util';
...
const getSibling = (el, distance) => {
    const { parentNode } = el;
    if (parentNode) {
        const siblings = parentNode.querySelectorAll('.aex-cascader-node[tabindex="-1"]');
    ...
    methods: {
        ...
        handleKeyDown(e) {
            ...
            switch (keyCode) {
                ...
                case KeyCode.left:
                    ...
                    if (preMenu) {
                        const expandedNode = preMenu.$el.querySelector('.aex-cascader-node[aria-expanded="true"]');
                ...
                case KeyCode.right:
                    ...
                    if (nextMenu) {
                        const firstNode = nextMenu.$el.querySelector('.aex-cascader-node[tabindex="-1"]');
                        focusNode(firstNode);
                    }
                    break;
                case KeyCode.enter:
        ...
        scrollIntoView() {
            ...
            menus.forEach((menu) => {
                ...
                if (menuElement) {
                    const container = menuElement.querySelector('.aex-scrollbar__wrap');
                    const activeNode =
                        menuElement.querySelector('.aex-cascader-node.is-active') ||
                        menuElement.querySelector('.aex-cascader-node.in-active-path');

<template>
	<div
		class="aex-select"
		:class="[selectSize ? 'aex-select--' + selectSize : '']"
		@click.stop="toggleMenu"
		v-clickoutside="handleClose"
	>
		<div
			class="aex-select__tags"
			v-if="multiple"
			ref="tags"
		...
		>
			<span v-if="collapseTags && selected.length">
				<aex-tag
					...
					disable-transitions
				>
					<span class="aex-select__tags-text">{{
		...
		>
			...
			<template slot="suffix">
				...
				<i
					v-if="showClose"
					class="aex-select__caret aex-input__icon aex-icon-error"
		...
		>
			...
			>
				...
				>
					<aex-option :value="query" created v-if="showNewOption">
					</aex-option>
					<slot></slot>
				</aex-scrollbar>
				<template
				...
				>
					...
					<p class="aex-select-dropdown__empty" v-else>
						{{ emptyText }}
					</p>
				</template>
...
import AexSelectMenu from './select-dropdown.vue';
import AexOption from 'aex-ui/packages/option';
...
} from 'aex-ui/src/utils/resize-event';
import scrollIntoView from 'aex-ui/src/utils/scroll-into-view';
import { getValueByPath, valueEquals } from 'aex-ui/src/utils/util';
import { isIE, isEdge } from 'aex-ui/src/utils/os';
		...
		autoComplete: {
			...
			validator(val) {
				process.env.NODE_ENV !== 'production' &&
					console.warn(
						"[Aex Warn][Select]'auto-complete' property will be deprecated in next major version. please use 'autocomplete' instead."
					);
				return true;
			}
		},

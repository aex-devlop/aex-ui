...
module.exports = require("aex-ui/lib/input-number");
...
var render = function() {
  ...
  return _c(
    "div",
    {
      staticClass: "aex-slider",
      class: {
        "is-vertical": _vm.vertical,
        "aex-slider--with-input": _vm.showInput
      },
      attrs: {
        role: "slider",
    ...
    [
      ...
      _c(
        ...
        {
          ref: "slider",
          staticClass: "aex-slider__runway",
        ...
        [
          _c("div", {
            staticClass: "aex-slider__bar",
            style: Object.assign({}, _vm.barStyle, {
              background: _vm.sliderDisabled ? "auto" : _vm.color
            })
          }),
          _c("slider-button", {
            ref: "button1",
            attrs: {
              ...
              color: _vm.sliderDisabled ? "auto" : _vm.color
            },
            model: {
              value: _vm.firstValue,
              callback: function($$v) {
          ...
          _vm._l(_vm.stops, function(item, key) {
            return _vm.showStops
              ? _c("div", {
                  key: key,
                  staticClass: "aex-slider__stop",
                  style: Object.assign(
                    {},
                    _vm.getStopStyle(item),
                    _vm.getStopColor(item)
                  )
                })
              : _vm._e()
          }),
            ...
            ? [
                _c(
                  ...
                  _vm._l(_vm.markList, function(item, key) {
                    return _c("div", {
                      key: key,
                      staticClass: "aex-slider__stop aex-slider__marks-stop",
                      style: Object.assign(
                        {},
                        _vm.getStopStyle(item.position),
                        _vm.getStopColor(item.position)
                      )
                    })
                  }),
                  0
...
// EXTERNAL MODULE: external "aex-ui/lib/input-number"
...
var buttonvue_type_template_id_e72d2ad2_render = function() {
  ...
  return _c(
    ...
    {
      ref: "button",
      staticClass: "aex-slider__button-wrapper",
    ...
    [
      _c(
        ...
        [
          ...
          _c("div", {
            staticClass: "aex-slider__button",
            class: { hover: _vm.hovering, dragging: _vm.dragging },
...
/* harmony default export */ var marker = ({
    ...
    render: function render() {
        ...
        return h(
            'div',
            { 'class': 'aex-slider__marks-text', style: this.mark.style || {} },
            [label]
        );
    }
});
...
/* harmony default export */ var mainvue_type_script_lang_js_ = ({
	...
	components: {
		AexInputNumber: input_number_default.a,
	...
	methods: {
		...
		setValues: function setValues() {
			if (this.min > this.max) {
				console.error('[Aex Error][Slider]min should not be greater than max.');
				return;
			}
			var val = this.value;
			if (this.range && Array.isArray(val)) {
				if (val[1] < this.min) {
					...
					if (this.valueChanged()) {
						this.dispatch('aexFormItem', 'el.form.change', [this.minValue, this.maxValue]);
				...
				if (val < this.min) {
					...
					this.firstValue = val;
					if (this.valueChanged()) {
						this.dispatch('aexFormItem', 'el.form.change', val);
						this.oldValue = val;
					}
				}
			}
		...
		getStopStyle: function getStopStyle(position, isRenderColor) {
			return this.vertical ? { bottom: position + '%' } : { left: position + '%' };
		},

		// top 在范围内的颜色
		getStopColor: function getStopColor(position) {
			if (this.sliderDisabled) return {};
			var size = parseFloat(this.barStyle[this.vertical ? 'height' : 'width'].split('%')[0]);
			var left = parseFloat(this.barStyle[this.vertical ? 'bottom' : 'left'].split('%')[0]);
			var background = position > left && position <= size + left ? this.color : this.unactiveStopColor;

<template>
	<div id="color">
		...
		<aex-text
			>整理了常用的颜色和阴影变量，使用时可以在全局引入UI变量,之后直接点击即可复制到剪贴版使用即可</aex-text
		>
		<pre>
        引入路径：@import 'aex-ui/packages/theme/src/common/var.less'
        </pre>
		<h3>主色调/品牌色</h3>
		...
		<div class="color-group">
			<div
				class="shadow-item shadow1"
				@click="_onCopy('@--box-shadow-1')"
			>
				<b>@--box-shadow-1</b
				><i> 0 1px 6px rgba(@--color-gray-dark-1, 10%)</i
				><span
					>S1基础投影，应用于默认状态的卡片、导航栏、组件里局部投影等。</span
				>
			</div>
			<div
				class="shadow-item shadow2"
				@click="_onCopy('@--box-shadow-2')"
			>
				<b>@--box-shadow-2</b
				><i>0 4px 12px rgba(@--color-gray-dark-1,10%)</i>
				<span
					>S2是升起投影，应用于悬停状态的卡片等，是S1的激活态。</span
				>
			</div>
			<div
				class="shadow-item shadow3"
				@click="_onCopy('@--box-shadow-3')"
			>
				<b>@--box-shadow-3</b
				><i>0 6px 18px rgba(@--color-gray-dark-1,12%)</i
				><span>S3是覆盖投影，应用于信息提示等投影。</span>
			</div>
			<div
				class="shadow-item shadow4"
				@click="_onCopy('@--box-shadow-4')"
			>
				<b>@--box-shadow-4</b
				><i>0 10px 32px rgba(@--color-gray-dark-1,24%)</i
...
export default {
	...
	data() {
		return {
			...
			warning: {
				'@--color-warning': ['#FD8900', '#FFF'],
				'@--color-warning-light-1': ['#FEDBB2', '#FA6400'],
				'@--color-warning-light-2': ['#FEF3E5', '#FA6400']
			},
			danger: {

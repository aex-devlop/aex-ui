import fecha from 'aex-ui/src/utils/date';
...
export const getI18nSettings = () => {
	return {
		dayNamesShort: weeks.map((week) => t(`aex.datepicker.weeks.${week}`)),
		dayNames: weeks.map((week) => t(`aex.datepicker.weeks.${week}`)),
		monthNamesShort: months.map((month) =>
			t(`aex.datepicker.months.${month}`)
		),
		monthNames: months.map((month, index) =>
			t(`aex.datepicker.month${index + 1}`)
		),
		amPm: ['am', 'pm']
	};
};
